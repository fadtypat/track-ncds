...
- **Admin (ผู้ดูแลระบบ)** — บทบาท `"admin"` ของ entity ผู้ใช้ (User) ใน
  ...
  เดียวกับแพทย์/พยาบาล (Operation 7) แล้วเรียก Operation 10-15 ด้านล่าง (อนุมัติบัญชี/เปลี่ยน role/
  ระงับ-เปิดใช้งานบัญชี/จัดการ PatientAssignment/ดูข้อมูลผู้ป่วยทุกรายแบบอ่านอย่างเดียว) — **ไม่มีสิทธิ์
  ...
  ที่มีอยู่แล้วเท่านั้น) หรือ Operation 16 (FR-16 — ไม่ใช่สิทธิ์ของ Admin) ใดๆ** และมีข้อยกเว้นจาก
  Operation ร่วม Access Control ตาม NFR-19 (อ่านข้อมูลผู้ป่วยทุกรายได้โดยไม่ต้องมี PatientAssignment
  ผ่าน Operation 1/2/3/15 เท่านั้น — ดูหมายเหตุในหัวข้อนั้นด้านล่าง)
...
ของเอกสารนี้) ส่วนงาน routine ทั้งหมด (อนุมัติบัญชี/เปลี่ยน role/ระงับ-เปิดใช้งาน/จัดการ
PatientAssignment/ดูข้อมูลผู้ป่วยทุกราย) ดำเนินการผ่าน Operation 10-15 แล้ว

ทุก operation ในเอกสารนี้ (Operation 0-6, 10-16) เรียกได้เฉพาะบทบาทที่ระบุไว้ของตนเท่านั้น และ
Operation 0-6/10-16 ทั้งหมดต้องผ่าน operation "ตรวจสอบสิทธิ์การเข้าถึงข้อมูลผู้ป่วย" (ด้านล่าง) ก่อน
...
- **กฎทางธุรกิจ:** แบ่งเป็น 2 ระดับตาม [[backlog#Non-Functional Requirements|NFR-02]] ฉบับขยายความ
  ...
  1. **ตรวจสอบระดับบทบาท (role-level)** — ตรวจสอบทุกครั้งไม่ว่าจะระบุรหัสผู้ป่วยหรือไม่: อนุญาตเฉพาะ
     เมื่อ User.บทบาท เป็น "แพทย์", "พยาบาล" **หรือ "admin" (เพิ่มใหม่ 2026-09-24 ตามฟีเจอร์ที่ 7)** และ
     User.สถานะการใช้งานบัญชี เป็นจริง — **ใหม่จากฟีเจอร์ที่ 6:** บัญชีที่เพิ่งสมัคร (Operation 8) แต่
     ยังไม่ได้รับอนุมัติจาก Admin มี User.บทบาท เป็นค่าว่าง/ไม่มีค่าและ User.สถานะการใช้งานบัญชี เป็นเท็จ
     โดยดีฟอลต์ (ดู [[db-spec#ผู้ใช้ (User)|db-spec]]) จึง**ไม่ผ่านการตรวจสอบระดับบทบาทนี้โดยอัตโนมัติ
     อยู่แล้ว** ไม่ต้องเพิ่มเงื่อนไขใหม่ (FR-08, NFR-02)
  2. **ตรวจสอบระดับรายผู้ป่วย (patient-level)** — ตรวจสอบเพิ่มเติมเฉพาะเมื่อมีการระบุรหัสผู้ป่วย:
     ...
     - **ข้อยกเว้นสำหรับบทบาท Admin (NFR-19 — เพิ่มใหม่ 2026-09-24):** เมื่อ User.บทบาท เป็น `"admin"`
       (และผ่านข้อ 1 กับข้อ 2.5 แล้ว) ให้**ข้าม**การตรวจสอบระดับรายผู้ป่วยนี้ **เฉพาะเมื่อเรียกผ่าน
       Operation 1, 2, 3 หรือ 15 เท่านั้น** (การอ่านข้อมูลผู้ป่วยแบบอ่านอย่างเดียวตาม FR-15) — Admin
       ยังคง**ไม่มีสิทธิ์**เรียก Operation 16 (FR-16 — ยืนยัน/แก้ไขผลประเมินความเสี่ยง เป็นสิทธิ์ของ
       แพทย์/พยาบาลเท่านั้น) หรือ operation ใดที่แก้ไขข้อมูลทางคลินิก แม้จะผ่านข้อยกเว้นนี้ก็ตาม —
       เมื่อผ่านข้อยกเว้นนี้แล้วต้องเรียก "บันทึกร่องรอยการเข้าถึงข้อมูลผู้ป่วย (Audit Logging)" ด้านล่าง
       พร้อมระบุ `เข้าถึงในฐานะ Admin หรือไม่ = จริง` เสมอก่อนคืนข้อมูล (fail-safe ตาม NFR-20)
...
- **กรณี error:**
  ...
  - บทบาทผู้ใช้ถูกต้อง (แพทย์/พยาบาล) แต่ระบุรหัสผู้ป่วยที่ไม่มี PatientAssignment เชื่อมโยงกับผู้ใช้นี้
    → ปฏิเสธการเข้าถึงข้อมูลผู้ป่วยรายนี้ (NFR-02, FR-05) — ผู้ใช้ยังคงเห็นรายชื่อผู้ป่วยรายอื่นที่ตน
    ดูแลอยู่ได้ปกติ — **ไม่ใช้กับ Admin** (ดูข้อยกเว้น NFR-19 ในกฎทางธุรกิจข้อ 2 ด้านบน)
  - Admin เรียก Operation 16 (FR-16) หรือ operation ที่แก้ไขข้อมูลทางคลินิกใดๆ → ปฏิเสธการเข้าถึง
    (ไม่ใช่สิทธิ์ของ Admin — เพิ่มใหม่ 2026-09-24)
...
- **Technical Binding (ตาม [[technology-stack#3. สถาปัตยกรรม Backend Service — Firebase-native (ไม่มี Backend Service แยกแบบดั้งเดิม)|decision area 3 ใน technology-stack]]):**
  ...
  - **สำหรับ Operation 1-6, 10-16:** เขียนเป็น shared helper module ภายในโค้ด Cloud Functions (Node.js +
    TypeScript) เรียกจากทุก callable function ก่อนดำเนินการ — ตรวจสอบบทบาท/สถานะบัญชีจาก Firestore
    `users/{uid}` (ค่าบทบาทที่ผ่านคือ `"แพทย์"`, `"พยาบาล"` หรือ `"admin"` — เพิ่ม `"admin"` ในรอบ
    sync ที่หก) และตรวจสอบ patient-level ผ่าน `exists()` บน `patientAssignments/{uid}_{patientId}`
    ด้วย Admin SDK (**ข้ามขั้นตอนนี้เมื่อ `role === 'admin'` และ operation ที่เรียกคือ Operation 1, 2,
    3 หรือ 15 เท่านั้น — ตาม NFR-19**) — **เพิ่มการตรวจสอบ `decodedToken.email_verified` ในโมดูลเดียวกัน
    ในรอบ 2026-09-24** (decision area 19) **ไม่มี custom claims บทบาท/isActive ให้อ่านจาก token อีก
    ต่อไป (decision area 18) — ต้อง query Firestore `users/{uid}` โดยตรงเสมอ**
...
- **Input:**
  ...
  - เข้าถึงในฐานะ Admin หรือไม่ (จริง/เท็จ) — ไม่บังคับ (ดีฟอลต์เท็จ; ระบุเป็นจริงเฉพาะเมื่อถูกเรียก
    จาก Operation 1, 2, 3 หรือ 15 โดยผู้ใช้ที่มีบทบาท `"admin"` ผ่านข้อยกเว้น NFR-19 — **ใหม่ในรอบ
    sync ที่หก, NFR-20**)
...
- **กฎทางธุรกิจ:**
  - ต้องบันทึกก่อนที่ Backend Service จะดึง/แก้ไขข้อมูลจริงจาก Primary Data Store เสมอ (NFR-06) —
    รวมถึงกรณี Admin เข้าถึงผ่านข้อยกเว้น NFR-19 (ต้องบันทึกก่อนคืนข้อมูลเช่นเดียวกัน ตาม NFR-20)
...
- **Technical Binding (ตาม [[technology-stack#5. Audit Log Store — Cloud Firestore collection แยก เขียนผ่าน Cloud Functions เท่านั้น|decision area 5 ใน technology-stack]]):**
  ...
  Operation 1, 2, 3, 4 หรือ 15 เอง (ไม่ใช่ operation แยกที่ Client เรียก) — เขียนระเบียนลง Firestore
  ...
  `isAdminAccess = true` เมื่อเรียกจาก Operation 1, 2, 3 หรือ 15 โดยผู้ใช้ `role === 'admin'` เท่านั้น
  ...
  (Operation 1, 2, 3, 4 หรือ 15) ทันที
...
รองรับ [[backlog#สูง (MVP)|FR-01]] และ [[backlog#สูง (MVP)|FR-15]] (Admin — เพิ่มใหม่ 2026-09-24)

- **ผู้เรียกได้/บทบาท:** แพทย์/พยาบาลผู้ดูแลผู้ป่วย NCD, **Admin (เฉพาะผู้ป่วยรายใดก็ได้ในระบบผ่าน
  ข้อยกเว้น NFR-19 — เพิ่มใหม่ 2026-09-24)**
- **Input:**
...
- **กฎทางธุรกิจ:**
  - ต้องผ่าน "ตรวจสอบสิทธิ์การเข้าถึงข้อมูลผู้ป่วย" ก่อนเสมอ (NFR-02) — เมื่อผู้เรียกเป็น Admin ให้ข้าม
    การตรวจสอบระดับรายผู้ป่วยตามข้อยกเว้น NFR-19 (ดูรายละเอียดในหัวข้อ operation ร่วมนั้น)
...
- **กรณี error:**
  - ไม่มีสิทธิ์เข้าถึง (บทบาทไม่ถูกต้อง หรือผู้ป่วยรายนี้ไม่ได้อยู่ในความดูแลของผู้ใช้งานคนนี้ตาม PatientAssignment — ไม่ใช้กับ Admin ซึ่งได้รับข้อยกเว้นตาม NFR-19) → ปฏิเสธการเข้าถึง (NFR-02, FR-05) พร้อมข้อความแจ้งผู้ใช้ตาม [[architecture]]
...
รองรับ [[backlog#สูง (MVP)|FR-02]] และ [[backlog#สูง (MVP)|FR-15]] (Admin — เพิ่มใหม่ 2026-09-24)
...
- **กฎทางธุรกิจ:**
  - เมื่อผู้เรียกเป็น Admin ให้ข้ามการตรวจสอบระดับรายผู้ป่วยตามข้อยกเว้น NFR-19 (ดูรายละเอียดใน
  ...
  - ข้อมูลอ้างอิงจากแหล่งข้อมูล HOSxP หรือข้อมูลจำลองระหว่างพัฒนา/ทดสอบ (NFR-01)
- **กรณี error:**
  - ไม่มีสิทธิ์เข้าถึง (บทบาทไม่ถูกต้อง หรือผู้ป่วยรายนี้ไม่ได้อยู่ในความดูแลของผู้ใช้งานคนนี้ตาม PatientAssignment — ไม่ใช้กับ Admin ซึ่งได้รับข้อยกเว้นตาม NFR-19) → ปฏิเสธการเข้าถึง (NFR-02, FR-05)
  - ไม่พบผู้ป่วยตามรหัสที่ระบุ → แจ้งว่าไม่พบผู้ป่วย
...
[[backlog#สูง (MVP)|FR-15]] (Admin — เพิ่มใหม่ 2026-09-24) — รวมเป็น operation เดียวเพราะผลการวิเคราะห์
ถูกส่งกลับให้ Client แสดงผลทันทีตาม sequence diagram ใน
[[architecture#Data Flow Diagram — Journey หลัก|architecture]]

- **ผู้เรียกได้/บทบาท:** แพทย์/พยาบาลผู้ดูแลผู้ป่วย NCD, **Admin (เฉพาะผู้ป่วยรายใดก็ได้ในระบบผ่าน
  ข้อยกเว้น NFR-19 — เพิ่มใหม่ 2026-09-24, อ่านอย่างเดียวเท่านั้น ไม่มีสิทธิ์เรียก Operation 16 ต่อ)**
- **Input:**
  - รหัสผู้ป่วย (Patient.id) — จำเป็น
  - ข้อมูลยืนยันตัวตน/บทบาทผู้ใช้ (auth context) — จำเป็น
...
- **กฎทางธุรกิจ:**
  - ต้องผ่าน "ตรวจสอบสิทธิ์การเข้าถึงข้อมูลผู้ป่วย" ก่อนเสมอ (NFR-02) — เมื่อผู้เรียกเป็น Admin ให้ข้าม
    การตรวจสอบระดับรายผู้ป่วยตามข้อยกเว้น NFR-19
...
## Operation 14 — จัดการการมอบหมายผู้ป่วย (Patient Assignment)

รองรับ [[backlog#สูง (MVP)|FR-14]] — **ใหม่จากฟีเจอร์ที่ 7** ปิดประเด็นรอตัดสินใจเดิมของ
[[db-spec#การมอบหมายผู้ป่วยในความดูแล (PatientAssignment)|PatientAssignment ใน db-spec]] เรื่องกลไก/
ผู้กำหนดการมอบหมาย

- **ผู้เรียกได้/บทบาท:** Admin เท่านั้น
- **Input:**
  - รหัสผู้ใช้เป้าหมาย (User.id ของแพทย์/พยาบาล) — จำเป็น
  - รหัสผู้ป่วย (Patient.id) — จำเป็น
  - การดำเนินการ ("มอบหมาย" หรือ "ยกเลิกการมอบหมาย") — จำเป็น
  - ข้อมูลยืนยันตัวตน/บทบาทผู้ใช้ (auth context) — จำเป็น
- **Output:** ยืนยันผลลัพธ์การดำเนินการ
- **กฎทางธุรกิจ:**
  - ผู้ใช้เป้าหมายต้องมี `บทบาท` เป็น "แพทย์" หรือ "พยาบาล" เท่านั้น (ไม่มอบหมายผู้ป่วยให้บัญชี Admin
    หรือบัญชีที่ยังไม่ผ่านการอนุมัติ)
  - กรณี "มอบหมาย": สร้างระเบียน [[db-spec#การมอบหมายผู้ป่วยในความดูแล (PatientAssignment)|PatientAssignment]]
    ใหม่ (ถ้ามีระเบียนเดิมอยู่แล้วสำหรับคู่ผู้ใช้-ผู้ป่วยนี้ ถือว่าสำเร็จโดยไม่สร้างซ้ำ — idempotent)
  - กรณี "ยกเลิกการมอบหมาย": ลบระเบียน PatientAssignment ที่เชื่อมโยงคู่ผู้ใช้-ผู้ป่วยนี้ทิ้ง (ถ้าไม่มี
    ระเบียนอยู่แล้ว ถือว่าสำเร็จโดยไม่มีผลใดๆ เพิ่ม — idempotent)
  - ต้องบันทึก audit log การเปลี่ยนแปลงการมอบหมายนี้เช่นเดียวกับกลุ่มงาน Audit Logging & Accountability
- **กรณี error:**
  - ผู้เรียกไม่ใช่ Admin หรือบัญชีถูกระงับ → ปฏิเสธการเข้าถึง (NFR-02)
  - ไม่พบผู้ใช้เป้าหมาย หรือไม่พบผู้ป่วยตามรหัสที่ระบุ → แจ้งว่าไม่พบข้อมูล
  - ผู้ใช้เป้าหมายมี `บทบาท` ไม่ใช่ "แพทย์"/"พยาบาล" (เช่น เป็น Admin หรือยังไม่ผ่านการอนุมัติ) → แจ้งว่า
    input ไม่ถูกต้อง
  - การดำเนินการที่ระบุไม่ใช่ "มอบหมาย"/"ยกเลิกการมอบหมาย" → แจ้งว่า input ไม่ถูกต้อง
- **อ้างอิง:** [[backlog#สูง (MVP)|FR-14]], [[backlog#สูง (MVP)|FR-05]],
  [[db-spec#การมอบหมายผู้ป่วยในความดูแล (PatientAssignment)|PatientAssignment]]
- **Technical Binding:** Cloud Functions (2nd gen, Node.js + TypeScript) — **HTTPS Callable Function
  ชื่อ `manageAssignedPatient`** — สร้าง/ลบเอกสาร `patientAssignments/{userId}_{patientId}` ผ่าน
  Firebase Admin SDK เท่านั้น (denormalize `patientHn`/`patientFullName` จาก `patients/{patientId}`
  ตอนสร้างเช่นเดียวกับที่ [[db-spec#การมอบหมายผู้ป่วยในความดูแล (PatientAssignment)|db-spec]] ระบุไว้) —
  **Error code:** ปฏิเสธการเข้าถึง → `permission-denied`; ไม่พบผู้ใช้/ผู้ป่วยเป้าหมาย → `not-found`;
  บทบาทผู้ใช้เป้าหมายไม่ถูกต้อง/การดำเนินการไม่ถูกต้อง → `invalid-argument`

## Operation 15 — ค้นหา/แสดงรายชื่อผู้ป่วยทั้งหมดในระบบ (สำหรับ Admin)

รองรับ [[backlog#สูง (MVP)|FR-15]] ส่วนการเลือกผู้ป่วยก่อนดูข้อมูล (precondition ของ Operation 1/2/3
เมื่อผู้เรียกเป็น Admin) — **ใหม่จากฟีเจอร์ที่ 7** ต่างจาก [[#Operation 0 — ค้นหา/แสดงรายชื่อผู้ป่วยในความดูแล (ค้นหาเฉพาะรายด้วยเลข HN)|Operation 0]]
ตรงที่**ไม่กรองด้วย PatientAssignment** (Admin เห็นผู้ป่วยทุกรายในระบบตาม NFR-19) และต้องผ่าน Cloud
Function เสมอ (ไม่ใช่ Client อ่าน Firestore ตรง เพราะ Security Rules ของ `patients` ปฏิเสธ Client
ทั้งหมดอยู่แล้ว)

- **ผู้เรียกได้/บทบาท:** Admin เท่านั้น
- **Input:**
  - เลข HN (เทียบกับ Patient.เลขประจำตัวผู้ป่วย) — ไม่บังคับ (ถ้าไม่ระบุ คืนรายชื่อผู้ป่วยทั้งหมดใน
    ระบบ; ถ้าระบุ ต้องเป็นค่าที่ผู้ใช้กรอกแล้ว "กดค้นหา" เท่านั้น เช่นเดียวกับหลักการของ FR-06)
  - ข้อมูลยืนยันตัวตน/บทบาทผู้ใช้ (auth context) — จำเป็น
- **Output:** รายการ Patient ทุกรายในระบบที่ตรงเงื่อนไข แต่ละรายการประกอบด้วย: id, เลขประจำตัวผู้ป่วย,
  ชื่อ-นามสกุล
- **กฎทางธุรกิจ:**
  - ต้องผ่านการตรวจสอบสิทธิ์ระดับบทบาทของ "ตรวจสอบสิทธิ์การเข้าถึงข้อมูลผู้ป่วย" ก่อนเสมอ (ตรวจว่า
    `role = "admin"` และ `isActive = true`) — **ไม่ตรวจสอบระดับรายผู้ป่วย/PatientAssignment ใดๆ**
    เพราะ Admin เห็นผู้ป่วยทุกรายตาม NFR-19
  - ถ้าระบุเลข HN ต้องตรวจสอบรูปแบบตัวเลขล้วน 7 หลักก่อนเช่นเดียวกับหลักการของ Operation 0/FR-06
  - ผลลัพธ์ใช้เป็น input ให้ Admin เลือกผู้ป่วยรายบุคคลก่อนเรียก Operation 1, 2, 3 ต่อ (FR-15)
- **กรณี error:**
  - ผู้เรียกไม่ใช่ Admin หรือบัญชีถูกระงับ → ปฏิเสธการเข้าถึง (NFR-02)
  - ระบุเลข HN แต่ไม่ครบรูปแบบตัวเลขล้วน 7 หลัก → แจ้งว่า input ไม่ถูกต้อง
  - ระบุเลข HN ครบ 7 หลักแล้ว แต่ไม่พบผู้ป่วยที่ตรงกันในระบบ → แจ้งเตือน "ไม่พบผู้ป่วย"
- **อ้างอิง:** [[backlog#สูง (MVP)|FR-15]], [[backlog#Non-Functional Requirements|NFR-19]],
  [[db-spec#ผู้ป่วย (Patient)|Patient]]
- **Technical Binding:** Cloud Functions (2nd gen, Node.js + TypeScript) — **HTTPS Callable Function
  ชื่อ `listAllPatientsForAdmin`** — อ่าน collection `patients` โดยตรงผ่าน Firebase Admin SDK (bypass
  Security Rules — ต่างจาก Operation 0 ที่ Client อ่าน `patientAssignments` ตรง) ใช้ composite index
  `(hn ASC)` เมื่อค้นหาด้วย HN — **Error code:** ปฏิเสธการเข้าถึง → `permission-denied`; HN ไม่ถูกต้อง
  → `invalid-argument`; ไม่พบผู้ป่วย → `not-found`
...
- **กฎทางธุรกิจ:**
  - ต้องผ่าน "ตรวจสอบสิทธิ์การเข้าถึงข้อมูลผู้ป่วย" ทั้งระดับบทบาทและระดับรายผู้ป่วยก่อนเสมอ (NFR-02) —
    ตรวจสอบว่าผู้ป่วยเจ้าของ ComplicationRiskAssessment นี้อยู่ในความดูแลของผู้เรียกจริง — **ปฏิเสธ
    เสมอถ้าผู้เรียกมี `role = "admin"`** (ไม่ใช่สิทธิ์ของ Admin แม้จะผ่านข้อยกเว้น NFR-19 สำหรับการอ่าน
    ก็ตาม)
  ...
  - เรียกได้เฉพาะเมื่อ `สถานะการยืนยัน/แก้ไข` ปัจจุบันเป็น "ยังไม่ดำเนินการ" เท่านั้น (ยืนยัน/แก้ไขได้
    ...
    เอกสาร)
- **กรณี error:**
  - ไม่มีสิทธิ์เข้าถึง (บทบาทไม่ถูกต้อง, ผู้ป่วยไม่ได้อยู่ในความดูแล, หรือผู้เรียกเป็น Admin) → ปฏิเสธ
    การเข้าถึง (NFR-02, FR-16)
...
| ผู้เรียกไม่ใช่ Admin | เฉพาะ Operation 10-15: ผู้เรียกไม่มี `role = "admin"` หรือบัญชีถูกระงับ | `permission-denied` | [[backlog#สูง (MVP)\|FR-11]]–[[backlog#สูง (MVP)\|FR-15]], [[backlog#Non-Functional Requirements\|NFR-02]] |
...
**ฟีเจอร์ที่ 7 (Admin) และ FR-16 — ปิดแล้วในรอบ sync ที่หก (2026-09-24):** สามจุดที่เคยเป็นประเด็นรอ
ตัดสินใจ (โครงสร้างข้อมูล FR-16, โครงสร้าง audit log NFR-20, การป้องกัน Admin ล็อกตัวเองออกจากสิทธิ์
Admin) ได้รับคำตอบยืนยันจากผู้ใช้แล้วผ่าน `NEEDS_USER_INPUT` และถูกนำไปออกแบบเป็น Operation 10-16
ครบแล้ว (ดูหัวข้อของแต่ละ operation ด้านบน) รายการที่**ยังไม่ตัดสินใจจริง**ในส่วนนี้:

- **จำนวน/การแบ่ง Cloud Function สำหรับ Operation 10-16** — เอกสารนี้ออกแบบให้แยกเป็น 7 callable
  function ตาม operation (`listUserAccounts`, `approveUserAccount`, `changeUserRole`,
  `setUserActiveStatus`, `manageAssignedPatient`, `listAllPatientsForAdmin`,
  `confirmOrOverrideRiskAssessment`) เพื่อความชัดเจนของ error code/permission ต่อ operation แยกกัน
  (สอดคล้องกับรูปแบบเดิมของเอกสารนี้ที่แยก Cloud Function ตาม FR) — แต่ยังไม่มี decision area ใน
  `[[technology-stack]]` ยืนยันจำนวนที่แน่นอน อาจรวมเป็นฟังก์ชันเดียวได้ในทางเทคนิคถ้าทีมพัฒนาต้องการ
  ลดจำนวน deployment unit — แนะนำให้รัน `/build-tech-stack` เพื่อยืนยัน
