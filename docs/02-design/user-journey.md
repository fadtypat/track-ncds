...
(NFR-09–NFR-16 ด้านคุณภาพเชิงปฏิบัติการ — cross-cutting ครอบคลุมทุกฟีเจอร์เช่นเดียวกับ PDPA) และ
...
ในหัวข้อ "บทบาทผู้ใช้ในขอบเขตนี้" ของ spec ต้นทาง — เอกสาร PDPA และเอกสาร Authentication ไม่ได้เพิ่ม
บทบาทใหม่ในระบบ) เอกสารนี้จึงมี 3 journey: journey แรกครอบคลุมฟีเจอร์ที่ 6 (สมัครบัญชี เข้าสู่ระบบ และ
จัดการรหัสผ่านด้วยอีเมล) ซึ่งเป็น precondition ก่อน journey อื่นทั้งหมด, journey ที่สองครอบคลุมฟีเจอร์ที่
1-3 ต่อเนื่องกัน (ค้นหา/เลือกผู้ป่วย แล้วดูประวัติ/ผล lab แล้วต่อด้วยผลวิเคราะห์ความเสี่ยง พร้อมการบันทึก
audit log ตามฟีเจอร์ที่ 4) และ journey ที่สามซึ่งครอบคลุมส่วนที่เหลือของฟีเจอร์ที่ 4 (คำขอใช้สิทธิของ
เจ้าของข้อมูล และการสนับสนุนการสืบสวนกรณีข้อมูลรั่วไหล) ที่ถูก trigger จากเหตุการณ์ภายนอก ไม่ใช่ routine
การดูแลผู้ป่วยแบบ journey ที่สอง

## Journey: ผู้ใช้งานสมัครบัญชี เข้าสู่ระบบ และจัดการรหัสผ่านด้วยอีเมล (Authentication)

บทบาท: แพทย์/พยาบาลผู้สมัครบัญชี/เข้าสู่ระบบ (ผู้ดูแลระบบเข้ามาเกี่ยวข้องเฉพาะขั้นตอนอนุมัติบัญชีนอก
ระบบผ่าน Firebase Console/Firestore โดยตรง ไม่ใช่บทบาท (role) ใหม่ในระบบ)
...
flowchart TD
    ...
    F --> G["รอผู้ดูแลระบบอนุมัติบัญชี กำหนด role และ isActive=true ผ่าน Firebase Console/Firestore (FR-08)"]
    G --> L1
...
6. ผู้ใช้งานยืนยันความเป็นเจ้าของอีเมลผ่านลิงก์ที่ได้รับ
   ([[20260923-01-user-authentication-email-password#ความต้องการเชิงฟังก์ชัน (Functional Requirements)|FR-09]])
7. บัญชีรอผู้ดูแลระบบกำหนด role (แพทย์/พยาบาล) และเปลี่ยน `isActive = true` ด้วยตนเองผ่าน Firebase
   Console/Firestore (ไม่มีหน้าจออนุมัติในระบบสำหรับ MVP — เป็นขั้นตอนเชิงองค์กรนอกระบบ) ก่อนจะเข้าถึง
   ข้อมูลผู้ป่วยได้ตาม NFR-02 — ไปขั้นตอนที่ 8 เพื่อเข้าสู่ระบบ
   ([[20260923-01-user-authentication-email-password#ความต้องการเชิงฟังก์ชัน (Functional Requirements)|FR-08]])
